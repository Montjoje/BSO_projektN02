...
<table><thead><tr><th>Ryzyko</th><th>Host</th><th>Nazwa / producent</th><th>Otwarte usługi</th><th>Liczba ustaleń</th></tr></thead><tbody>
{{range .Hosts}}
<tr><td class="risk-{{.RiskLevel}}">{{.RiskLevel}} ({{.RiskScore}} pkt)</td><td><span class="code">{{.IP}}</span></td><td>{{or .Hostname "-"}}<br><span class="muted">{{or .Vendor "-"}}</span></td><td>{{range .Services}}{{if eq .State "open"}}<span class="code">{{.Protocol}}/{{.Port}} {{.Name}}</span><br>{{end}}{{end}}</td><td>{{len .Findings}}</td></tr>
{{end}}
</tbody></table>
...
func RenderText(result models.ScanResult) string {
	...
	fmt.Fprintf(&b, "Podsumowanie: hosty=%d, otwarte porty=%d, ustalenia=%d, high=%d, medium=%d, low=%d\n\n",
		result.Summary.HostCount, result.Summary.OpenPortCount, result.Summary.FindingCount, result.Summary.HighRiskHosts, result.Summary.MediumRiskHosts, result.Summary.LowRiskHosts)
